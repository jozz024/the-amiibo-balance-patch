...
use smash::app::lua_bind::{AttackModule, VisibilityModule, WorkModule};
...
unsafe fn samus_downb(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
    let boma = smash::app::sv_system::battle_object_module_accessor(lua_state);
    ...
    if macros::is_excute(fighter) {
        WorkModule::on_flag(
            boma,
            /*Flag*/ *FIGHTER_SAMUS_STATUS_SPECIAL_LW_FLAG_JUMP,
        );
    }
    frame(lua_state, 10.0);
    if macros::is_excute(fighter) {
    ...
    if macros::is_excute(fighter) {
        WorkModule::on_flag(
            boma,
            /*Flag*/ *FIGHTER_SAMUS_STATUS_SPECIAL_LW_FLAG_WEAPON,
        );
        VisibilityModule::set_int64(
            boma,
            Hash40::new("body").hash.try_into().unwrap(),
            Hash40::new("body_sphere").hash.try_into().unwrap(),
        );
    ...
    if macros::is_excute(fighter) {
        VisibilityModule::set_int64(
            boma,
            Hash40::new("body").hash.try_into().unwrap(),
            Hash40::new("body_normal").hash.try_into().unwrap(),
        );
    }
    frame(lua_state, 45.0);
    if macros::is_excute(fighter) {
        WorkModule::on_flag(
            boma,
            /*Flag*/ *FIGHTER_SAMUS_STATUS_SPECIAL_LW_FLAG_CHK_CROUCH,
        );
    }
    macros::FT_MOTION_RATE(fighter, 0.2);
}
pub fn install() {
    smashline::install_acmd_scripts!(samus_fsmash, samus_dsmash, samus_ftilt, samus_downb);
}
